function [decoded] = decoder2(y, N)
%DECODER2 Decode the data encoded with the E2 convolutional encoder
%   decode the vector encoded with G = 1+D^2+D^3+D^4, 1+D^2+D^3
...
% initialisation
dist(1,1) = 0; % we start from state 0000
dist((2:16),1) = Inf; % all other state are at an infinite distance

for i=1:N
    % compute branch metrics
    transit(1,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2);  % from 0000 to 0000
    transit(2,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2);  % from 0000 to 1000
    transit(3,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2);  % from 0001 to 0000
    transit(4,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2);  % from 0001 to 1000
    transit(5,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2);  % from 0010 to 0001
    transit(6,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2);  % from 0010 to 1001
    transit(7,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2);  % from 0011 to 0001
    transit(8,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2);  % from 0011 to 1001
    transit(9,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2);  % from 0100 to 0010
    transit(10,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2); % from 0100 to 1010
    transit(11,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2); % from 0101 to 0010
    transit(12,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2); % from 0101 to 1010
    transit(13,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2); % from 0110 to 0011
    transit(14,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2); % from 0110 to 1011
    transit(15,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2); % from 0111 to 0011
    transit(16,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2); % from 0111 to 1011
    transit(17,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2); % from 1000 to 0100
    transit(18,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2); % from 1000 to 1100
    transit(19,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2); % from 1001 to 0100
    transit(20,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2); % from 1001 to 1100
    transit(21,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2); % from 1010 to 0101
    transit(22,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2); % from 1010 to 1101
    transit(23,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2); % from 1011 to 0101
    transit(24,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2); % from 1011 to 1101
    transit(25,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2); % from 1100 to 0110
    transit(26,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2); % from 1100 to 1110
    transit(27,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2); % from 1101 to 0110
    transit(28,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2); % from 1101 to 1110
    transit(29,i) = sqrt((0 - y0(i))^2 + (0 - y1(i))^2); % from 1110 to 0111
    transit(30,i) = sqrt((1 - y0(i))^2 + (1 - y1(i))^2); % from 1110 to 1111
    transit(31,i) = sqrt((1 - y0(i))^2 + (0 - y1(i))^2); % from 1111 to 0111
    transit(32,i) = sqrt((0 - y0(i))^2 + (1 - y1(i))^2); % from 1111 to 1111
...
%decoded;
%BitErrs = sum(a ~= decoded) % count the bit errors and evaluate the bit error rat
end
